...
import {showFirstTimeLoginSuccessModal, showFirstTimeLoginErrorModal, showPasswordResetSuccessModal, showPasswordResetErrorModal} from "./ui";
...
export function configure(endpoint = {}, settings = {}) {
  return dispatch => {
    ...
    if (settings.isServer) {
      promise = verifyAuth(endpoint, settings).then(({
        user,
        headers,
        firstTimeLogin,
        mustResetPassword,
        ...
        dispatch(ssAuthTokenUpdate({headers, user, firstTimeLogin, mustResetPassword}));

        dispatch(setEndpointKeys(Object.keys(currentEndpoint), currentEndpointKey, defaultEndpointKey));

        return user;
      }).catch(({reason, firstTimeLogin, mustResetPassword, currentEndpoint, defaultEndpointKey}) => {
      ...
      if (tokenBridge) {
        ...
        if (rawServerCreds) {
          ...
          dispatch(ssAuthTokenUpdate({user, headers, mustResetPassword, firstTimeLogin}));
        }
      }
      ...
      // if tokens were invalidated by server or from the settings, make sure to clear
      // browser credentials
